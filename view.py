...
class OrderFrame(tk.Frame):
    def __init__(self, parent, model: AppModel, order_id: int = 0, **kwargs):
        ...
        self.combo_symbol = ttk.Combobox(self, width=25)
        ...
        # ---------------- Quantity + Risk ----------------
        ttk.Label(self, text="Quantity").grid(row=2, column=0)
        self.entry_qty = ttk.Entry(self, width=10)
        self.entry_qty.grid(row=2, column=1, padx=5)

        ttk.Label(self, text="Stop Loss").grid(row=2, column=2)
        self.entry_sl = ttk.Entry(self, width=10)
        self.entry_sl.grid(row=2, column=3, padx=5)

        ttk.Label(self, text="Profit Taking %").grid(row=2, column=4)
        self.entry_tp = ttk.Entry(self, width=10)
        self.entry_tp.grid(row=2, column=5, padx=5)

        # ---------------- Control Buttons ----------------
        frame_ctrl = ttk.Frame(self)
        frame_ctrl.grid(row=3, column=0, columnspan=8, pady=10)
    ...
    def on_symbol_typed(self, event):
        ...
        try:
            ...
            if results:
                symbols = [f"{r['symbol']} - {r['primaryExchange']}" for r in results]
